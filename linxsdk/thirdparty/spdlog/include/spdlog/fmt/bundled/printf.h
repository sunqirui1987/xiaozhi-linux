...
void vprintf(buffer<Char>& buf, basic_string_view<Char> format, basic_format_args<Context> args) {
    ...
    while (it != end) {
        ...
        if (it != end && *it == c) {
            out =
                detail::write(out, basic_string_view<Char>(start, detail::to_unsigned(it - start)));
            start = ++it;
            continue;
        }
        out =
            detail::write(out, basic_string_view<Char>(start, detail::to_unsigned(it - 1 - start)));
        ...
        if (specs.precision >= 0 && arg.type() == detail::type::cstring_type) {
            ...
            arg = detail::make_arg<basic_printf_context<OutputIt, Char>>(basic_string_view<Char>(
                str, detail::to_unsigned(nul != str_end ? nul - str : specs.precision)));
        }
        if (specs.alt && visit_format_arg(detail::is_zero_int(), arg)) specs.alt = false;
        if (specs.fill[0] == '0') {
        ...
        // Format argument.
        out = visit_format_arg(detail::printf_arg_formatter<OutputIt, Char>(out, specs, context),
                               arg);
    }
...
inline auto vsprintf(const S& fmt,
...
template <typename S, typename Char = char_t<S>>
inline auto vfprintf(std::FILE* f, const S& fmt,
...
template <typename S, typename Char = char_t<S>>
FMT_DEPRECATED auto vfprintf(std::basic_ostream<Char>& os, const S& fmt,
                             basic_format_args<basic_printf_context_t<type_identity_t<Char>>> args)
    -> int {
    basic_memory_buffer<Char> buffer;
    vprintf(buffer, to_string_view(fmt), args);
