...
template <typename T>
constexpr int16_t basic_impl_data<T>::pow10_exponents[];
template <typename T>
constexpr uint64_t basic_impl_data<T>::power_of_10_64[];
#endif
...
struct fp {
    ...
    template <typename Float>
    explicit FMT_CONSTEXPR fp(Float n) {
        assign(n);
    }
    ...
    FMT_CONSTEXPR bool assign(Float n) {
        ...
        const int num_float_significand_bits = detail::num_significand_bits<Float>();
        ...
        int biased_e = static_cast<int>((u & exponent_mask) >> num_float_significand_bits);
...
FMT_CONSTEXPR fp normalize(fp value) {
    ...
    const auto offset = fp::num_significand_bits - num_significand_bits<double>() - SHIFT - 1;
    value.f <<= offset;
    value.e -= offset;
    return value;
}
...
FMT_CONSTEXPR inline fp get_cached_power(int min_exponent, int& pow10_exponent) {
    ...
    return {impl_data::pow10_significands[index], impl_data::pow10_exponents[index]};
}
...
public:
    ...
    FMT_CONSTEXPR20 ~bigint() { FMT_ASSERT(bigits_.capacity() <= bigits_capacity, ""); }
...
FMT_CONSTEXPR inline round_direction get_round_direction(uint64_t divisor, uint64_t remainder,
                                                         uint64_t error) {
...
struct gen_digits_handler {
    ...
    FMT_CONSTEXPR digits::result on_digit(char digit, uint64_t divisor, uint64_t remainder,
                                          uint64_t error, bool integral) {
...
FMT_INLINE FMT_CONSTEXPR20 digits::result grisu_gen_digits(fp value, uint64_t error, int& exp,
    ...
    if (handler.fixed) {
        ...
        if (precision_offset > 0 && handler.precision > max_value<int>() - precision_offset) {
    ...
    do {
        ...
        auto result =
            handler.on_digit(static_cast<char>('0' + digit),
                             impl_data::power_of_10_64[exp] << -one.e, remainder, error, true);
...
struct uint128_wrapper {
    ...
    constexpr uint128_wrapper(uint64_t high, uint64_t low) FMT_NOEXCEPT : high_{high}, low_{low} {}
...
struct cache_accessor<double> {
    ...
    static uint128_wrapper get_cached_power(int k) FMT_NOEXCEPT {
        ...
        uint32_t error =
            (pow10_recovery_errors[error_idx] >> ((k - float_info<double>::min_k) % 16) * 2) & 0x3;
    ...
    static carrier_uint compute_round_up_for_shorter_interval_case(const cache_entry_type& cache,
                                                                   int beta_minus_1) FMT_NOEXCEPT {
        return ((cache.high() >> (64 - float_info<double>::significand_bits - 2 - beta_minus_1)) +
                1) /
               2;
    }
};
...
FMT_INLINE int remove_trailing_zeros(uint32_t& n) FMT_NOEXCEPT {
    ...
    int t = ctz(n);
#endif
    if (t > float_info<float>::max_trailing_zeros) t = float_info<float>::max_trailing_zeros;
...
FMT_INLINE int remove_trailing_zeros(uint64_t& n) FMT_NOEXCEPT {
    ...
    int t = ctzll(n);
#endif
    if (t > float_info<double>::max_trailing_zeros) t = float_info<double>::max_trailing_zeros;
...
FMT_INLINE decimal_fp<T> shorter_interval_case(int exponent) FMT_NOEXCEPT {
    ...
    auto xi =
        cache_accessor<T>::compute_left_endpoint_for_shorter_interval_case(cache, beta_minus_1);
    auto zi =
        cache_accessor<T>::compute_right_endpoint_for_shorter_interval_case(cache, beta_minus_1);
...
FMT_CONSTEXPR20 inline void format_dragon(fp value, bool is_predecessor_closer, int num_digits,
...
template <typename Float>
FMT_HEADER_ONLY_CONSTEXPR20 int format_float(Float value, int precision, float_specs specs,
                                             buffer<char>& buf) {
...
FMT_FUNC void detail::error_handler::on_error(const char* message) { throw_format_error(message); }
