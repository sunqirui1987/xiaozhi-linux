...
#pragma clang diagnostic pop
    ...
    auto result = f.in(mb, in_buf.begin(), in_buf.end(), from_next, std::begin(out.buf),
...
inline void do_write(buffer<Char>& buf, const std::tm& time, const std::locale& loc, char format,
                     char modifier) {
...
public:
    ...
    void on_us_date() {
        char buf[8];
        write_digit2_separated(buf, to_unsigned(tm_mon() + 1), to_unsigned(tm_mday()),
    ...
    void on_12_hour(numeric_system ns) {
        if (is_classic_ || ns == numeric_system::standard) return write2(tm_hour12());
    ...
    void on_12_hour_time() {
        if (is_classic_) {
            char buf[8];
            write_digit2_separated(buf, to_unsigned(tm_hour12()), to_unsigned(tm_min()),
                                   to_unsigned(tm_sec()), ':');
            out_ = copy_str<Char>(std::begin(buf), std::end(buf), out_);
            *out_++ = ' ';
            on_am_pm();
...
constexpr int count_fractional_digits(long long num, long long den, int n = 0) {
    return num % den == 0 ? n : (n > 18 ? 6 : count_fractional_digits(num * 10, den, n + 1));
}
    ...
    void write_fractional_seconds(Duration d) {
        ...
        if (std::ratio_less<typename subsecond_precision::period,
            ...
            *out++ = '.';
            ...
            using sec = conditional_t<std::is_same<typename Duration::rep, long double>::value,
...
struct formatter<std::chrono::time_point<std::chrono::system_clock, Duration>, Char>
    ...
    FMT_CONSTEXPR formatter() {
        this->do_parse(default_specs, default_specs + sizeof(default_specs) / sizeof(Char));
    }
    ...
    template <typename FormatContext>
    auto format(std::chrono::time_point<std::chrono::system_clock> val, FormatContext& ctx) const
        -> decltype(ctx.out()) {
...
protected:
    ...
    FMT_CONSTEXPR auto do_parse(It begin, It end, bool with_default = false) -> It {
        ...
        if (!with_default || end != begin) specs = {begin, detail::to_unsigned(end - begin)};
